...
+ modules complémentaires : qualité de l'air, actus du monde, bourse (Yahoo Finance), transports (Navitia)."""
import datetime
...
from config import NAVITIA_API_KEY, SUPABASE_URL, SUPABASE_ANON_KEY
...
NEWS_RSS_URL = "https://www.lemonde.fr/international/rss_full.xml"

# API non officielle mais gratuite et sans clé, utilisée pour les indices, actions et cryptos.
...
# Préréglages de période : (plage Yahoo, granularité, nb de points à garder, format d'heure)
RANGE_PRESETS = {
    "1h": {"range": "1d", "interval": "1m", "label": "Dernière heure", "keep_last": 60},
    "1d": {"range": "1d", "interval": "5m", "label": "Journée", "keep_last": None},
    "1w": {"range": "5d", "interval": "15m", "label": "Semaine", "keep_last": None},
    "1y": {"range": "1y", "interval": "1d", "label": "Année", "keep_last": None},
    "10y": {"range": "10y", "interval": "1wk", "label": "Décennie", "keep_last": None},
}
...
def api_news():
    try:
        resp = requests.get(NEWS_RSS_URL, headers=YAHOO_HEADERS, timeout=10)
        resp.raise_for_status()
        root = ET.fromstring(resp.content)
    except (requests.RequestException, ET.ParseError):
        return jsonify({"error": "Impossible de récupérer les actualités."}), 502
...
def yahoo_get(url, params):
    resp = requests.get(url, params=params, headers=YAHOO_HEADERS, timeout=10)
    resp.raise_for_status()
    return resp.json()


@app.route("/api/stocks/majors")
def api_stocks_majors():
    return jsonify({"indices": MAJOR_INDICES})


@app.route("/api/stocks/currencies")
def api_stocks_currencies():
    return jsonify({"currencies": MAJOR_CURRENCIES})
...
def api_stocks_search():
    ...
    try:
        data = yahoo_get(YAHOO_SEARCH_URL, {"q": query, "quotesCount": 8, "newsCount": 0})
    except requests.RequestException:
        return jsonify({"error": "Impossible de contacter le service financier."}), 502

    results = []
    for quote in data.get("quotes", []):
        if quote.get("quoteType") not in ("EQUITY", "ETF", "INDEX", "CRYPTOCURRENCY"):
            continue
        results.append({
...
def api_stocks_quote():
    ...
    if not symbol:
        return jsonify({"error": "Merci d'indiquer un symbole."}), 400

    preset = RANGE_PRESETS.get(period, RANGE_PRESETS["1d"])

    try:
        data = yahoo_get(
            YAHOO_CHART_URL.format(symbol=symbol),
            {"range": preset["range"], "interval": preset["interval"]},
        )
    except requests.RequestException:
        return jsonify({"error": "Impossible de contacter le service financier."}), 502
    ...
    for ts, price in zip(timestamps, closes):
        if price is None:
            continue
        local_dt = datetime.datetime.fromtimestamp(ts + gmtoffset, tz=datetime.timezone.utc)
        if preset["interval"] in ("1m", "5m", "15m"):
            label = local_dt.strftime("%H:%M")
        elif preset["interval"] in ("1d",):
            label = local_dt.strftime("%d/%m")
        else:
            label = local_dt.strftime("%m/%Y")
        points.append({"label": label, "price": round(price, 4)})

    if preset.get("keep_last"):
        points = points[-preset["keep_last"]:]

    price = meta.get("regularMarketPrice")
    previous_close = meta.get("chartPreviousClose") or meta.get("previousClose")
    change = round(price - previous_close, 4) if price is not None and previous_close else None
...
def api_transit_stops():
    ...
    try:
        nearby = navitia_get(
            ...
            {"type[]": "stop_area", "distance": 3000, "count": 6},
        )
    except requests.RequestException:
        return jsonify({"error": "Impossible de contacter le service de transport (Navitia)."}), 502

    stops = [
        {"id": place.get("id"), "name": place.get("name"), "distance": place.get("distance")}
        for place in nearby.get("places_nearby", [])
    ]
    return jsonify({"stops": stops})


@app.route("/api/transit/departures")
def api_transit_departures():
    stop_id = request.args.get("stop_id", "").strip()
    if not stop_id:
        return jsonify({"error": "Merci d'indiquer un arrêt."}), 400

    try:
        deps = navitia_get(f"/stop_areas/{stop_id}/departures", {"count": 8})
    except requests.RequestException:
        return jsonify({"error": "Impossible de contacter le service de transport (Navitia)."}), 502
